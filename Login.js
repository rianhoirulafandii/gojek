...
(async () => {
    ...
    try{
		...
		if (checkacc.errors[0].code === `GPS-Proxy-CustomerBlocked`) {
            ...
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEK.txt',`${phoneNumber}|${aksestoken}|${pins}|DIED\n`, function (err) {
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
        }else{
        console.log(chalk.green(`[-] Hooray! Your account is LIVE `))
        const pins = "080599";
        const badlist = await fs.appendFile('ALL-SEWA2.txt',`${phoneNumber}|${aksestoken}|${pins}|LIVE\n`, function (err) {
        if (err) throw err;
            console.log(chalk.red('[!] Failed to save list, boss!'));});
		const kodepromonya = "GOPAYMLCC";
        ...
        if (redeemvoc.success === false) {
		    ...
		    const badlist = await fs.appendFile('MLCC.txt',`${aksestoken}\n`);
        };
		// await delay(5000);
		const kodepromonya3 = "COBAGOPAYMLCC220";
