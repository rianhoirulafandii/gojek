...
(async () => {
    ...
    try{
		...
		if (checkacc.errors[0].code === `GPS-Proxy-CustomerBlocked`) {
            ...
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEKACCOUNT.txt',`${phoneNumber}|${aksestoken}|${pins}|DIED\n`, function (err) {
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
        }else{
        console.log(chalk.green(`[-] Hooray! Your account is LIVE `))
        const kodepromonya = "SAHURCODA";
        const redeemvoc = await functionredeemvoc2(sessionnya, uniknya, aksestoken, kodepromonya);
        if (redeemvoc.success === true) {
            console.log(chalk.yellow(`[+] Successfully redeem code : ${kodepromonya}`));
            console.log(chalk.green(`[+] ${redeemvoc.data.message}`));
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEKACCOUNT.txt',`${phoneNumber}|${aksestoken}|${pins}|LIVE\n`, function (err) {
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
            ...
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
            await delay(5000)
        }else if (redeemvoc.errors[0].code === 'GPS-Proxy-CustomerBlocked') {
            console.log(chalk.red(`[!] ${redeemvoc.errors[0].message}`));
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEKACCOUNT.txt',`${phoneNumber}|${aksestoken}|${pins}|DIED\n`, function (err) {
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
        }else if (redeemvoc.errors[0].code === 'GoPromo-FailureLimitReached') {
            console.log(chalk.red(`[!] ${redeemvoc.errors[0].message}`));
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEKACCOUNT.txt',`${phoneNumber}|${aksestoken}|${pins}|LIVE\n`, function (err) {
            if (err) throw err;
                console.log(chalk.red('[!] Failed to save list, boss!'));});
            await delay(5000)
        }else if (redeemvoc.errors[0].code === 'GoPromo-InvalidSegment') {
            console.log(chalk.red(`[!] ${redeemvoc.errors[0].message}`));
            const pins = "080599";
            const badlist = await fs.appendFile('ALL-GOJEKACCOUNT.txt',`${phoneNumber}|${aksestoken}|${pins}|LIVE\n`, function (err) {
            if (err) throw err;
		...
		await delay(5000);
		const kodepromonya3 = "NEWMLBBRM20";
        ...
        if (redeemvoc3.success === false) {
			...
			const badlist = await fs.appendFile('GOOGLEPLAY90.txt',`${aksestoken}\n`);
            };
        };
